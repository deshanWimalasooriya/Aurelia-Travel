...
const DashboardHotels = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', 
    address: '',        // Maps to address_line_1
    city: '', 
    province: '',       // Required by DB
    postalCode: '',     // Required by DB
    country: '',
    description: '', 
    imageUrl: '',       // Maps to image_url
    facilities: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // --- FIX 1: Map fields to match Database Schema ---
    const payload = {
      name: formData.name,
      address_line_1: formData.address, // Fix: address -> address_line_1
      city: formData.city,
      province: formData.province || formData.city, // Fallback
      postal_code: formData.postalCode || '00000',  // Fallback
      country: formData.country,
      description: formData.description,
      image_url: formData.imageUrl,     // Fix: imageUrl -> image_url
      facilities: formData.facilities.split(',').map(f => f.trim()).filter(Boolean)
    };

    try {
      // --- FIX 2: Add { withCredentials: true } to send cookies ---
        ...
        {view === 'form' && (
          ...
          >
            ...
            <form onSubmit={handleSubmit}>
                ...
                {/* Location Row 2 (New Required Fields) */}
                <div className="form-grid-row">
                    <div className="form-group">
